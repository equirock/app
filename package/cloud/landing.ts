...
const landingConfig = new pulumi.Config("landing");
const landingPackageName = "equirock--package-landing";

const domain = landingConfig.require("API_DOMAIN");
...
const bucketOwnershipControls = new aws.s3.BucketOwnershipControls(
  landingPackageName + "bucketOwnershipControls",
  {
...
const bucketPublicAccessBlock = new aws.s3.BucketPublicAccessBlock(
  landingPackageName + "bucketPublicAccessBlock",
  {
    bucket: contentBucket.id,
...
new aws.s3.BucketAclV2(
  landingPackageName + "bucketAclV2",
  {
    bucket: contentBucket.id,
    acl: "public-read",
  },
...
const contentBucketWebsiteConf = new aws.s3.BucketWebsiteConfigurationV2(
  landingPackageName + "-contentBucketWebsiteConf",
  {
    bucket: contentBucket.id,
    indexDocument: {
      suffix: "index.html",
...
const contentBucketPolicy = new aws.s3.BucketPolicy(
  landingPackageName + "-contentBucketPolicy",
  {
    bucket: contentBucket.id,
...
const awsUsEast1 = new aws.Provider(landingPackageName + "usEast1", {
  region: "us-east-1",
});

const certificate = new aws.acm.Certificate(
  landingPackageName + "-sslCert",
  {
    domainName: domain,
...
const certificateValidationDomain = new aws.route53.Record(
  landingPackageName + `-${domain}-validation`,
  {
...
const subdomainCertificateValidationDomain = new aws.route53.Record(
  landingPackageName + `${domain}-validation2`,
  {
...
const certificateValidation = new aws.acm.CertificateValidation(
  landingPackageName + "-certificateValidation",
  {
...
const distributionArgs: DistributionArgs = {
  enabled: true,
...
new aws.route53.Record(landingPackageName + `-${domain}-www-alias`, {
  name: `www.${domain}`,
  zoneId: hostedZoneId,
  type: "A",
  aliases: [
